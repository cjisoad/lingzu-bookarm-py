...
class ELA3Kinematics:
    ...
    def __init__(
        ...
        self._tcp_offset = pin.SE3.Identity()
    ...
    def forward_kinematics(self, q: List[float]) -> ArmEndPose:
        ...
        oMf = self._update_frame_placement(q_pin)
        oMtcp = oMf * self._tcp_offset
        pos = oMtcp.translation
        rpy = pin.rpy.matrixToRpy(oMtcp.rotation)
    ...
    def forward_kinematics_se3(self, q: List[float]) -> "pin.SE3":
        ...
        q_pin = self._to_pinocchio_q(q)
        return self._update_frame_placement(q_pin) * self._tcp_offset

    def get_tcp_offset(self) -> List[float]:
        """返回当前 TCP 偏移 [x, y, z, rx, ry, rz]（m / rad）"""
        return self._se3_to_tcp_offset(self._tcp_offset)

    def set_tcp_offset(self, tcp_offset: Optional[Sequence[float]] = None) -> List[float]:
        """设置 TCP 偏移 [x, y, z, rx, ry, rz]（m / rad）"""
        values = self._normalize_tcp_offset(tcp_offset)
        self._tcp_offset = self._tcp_offset_to_se3(values)
        return list(values)

    def reset_tcp_offset(self) -> List[float]:
        """恢复默认 TCP 偏移（当前末端点）"""
        return self.set_tcp_offset(None)
    ...
    def inverse_kinematics(
        ...
        """
        target_se3 = self._pose_to_se3(target_pose) * self._tcp_offset.inverse()

        q = self._to_pinocchio_q(q_init) if q_init else pin.neutral(self._model)

        for i in range(max_iter):
            oMf = self._update_frame_placement(q)
    ...
    def compute_jacobian(self, q: List[float]) -> np.ndarray:
        ...
        r_world = oMf.rotation @ self._tcp_offset.translation
        J_tcp = J.copy()
        J_tcp[:3, :] -= self._skew(r_world) @ J_tcp[3:, :]
        return J_tcp
    ...
    def _update_frame_placement(self, q_pin: np.ndarray) -> "pin.SE3":
        pin.forwardKinematics(self._model, self._data, q_pin)
        pin.updateFramePlacements(self._model, self._data)
        return self._data.oMf[self._ee_frame_id]

    @staticmethod
    def _normalize_tcp_offset(tcp_offset: Optional[Sequence[float]]) -> List[float]:
        values = [0.0] * 6
        ...
        for idx in range(min(6, len(seq))):
            try:
                values[idx] = float(seq[idx])
            except (TypeError, ValueError):
                values[idx] = 0.0
        return values

    @staticmethod
    def _tcp_offset_to_se3(offset: Sequence[float]) -> "pin.SE3":
        x, y, z, rx, ry, rz = ELA3Kinematics._normalize_tcp_offset(offset)
        R = pin.rpy.rpyToMatrix(rx, ry, rz)
        t = np.array([x, y, z], dtype=float)
        return pin.SE3(R, t)

    @staticmethod
    def _se3_to_tcp_offset(offset: "pin.SE3") -> List[float]:
        pos = offset.translation
        rpy = pin.rpy.matrixToRpy(offset.rotation)
        return [
